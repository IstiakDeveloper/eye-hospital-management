...
<body>
    ...
    <div class="header">
        <h1>EYE HOSPITAL - MEDICINE SALES REPORT</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            @forelse($sales as $sale)
                @php
                    $dueAmount = $sale->total_amount - $sale->paid_amount;
                    $grandTotal += $sale->total_amount;
                    $grandCashReceive += $sale->paid_amount;
                    $grandDue += $dueAmount;
                @endphp
                <tr>
                    <td class="text-center">{{ $serialNo++ }}</td>
                    <td><strong>{{ $sale->invoice_number }}</strong></td>
                    <td>{{ $sale->customer_name ?? ($sale->patient->name ?? 'Walk-in Customer') }}</td>
                    ...
                    <td class="text-right">
                        <strong style="color: {{ $dueAmount > 0 ? '#dc2626' : '#16a34a' }};">
