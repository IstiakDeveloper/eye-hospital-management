...
const FlashMessage: React.FC<FlashMessageProps> = ({ type, message, onClose }) => {
    ...
    return (
        <div
            className={`fixed top-4 right-4 z-50 w-full max-w-md transform transition-all duration-300 ease-in-out ${
        ...
        <div className="fixed top-0 right-0 z-50 space-y-2 p-4">
            {messages.map((message, index) => (
                <div key={message.id} style={{ marginTop: `${index * 70}px` }}>
                    <FlashMessage type={message.type} message={message.message} onClose={() => removeMessage(message.id)} />
                </div>
            ))}
        </div>
    );
