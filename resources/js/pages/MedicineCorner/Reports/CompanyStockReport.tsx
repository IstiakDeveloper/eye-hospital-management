    ...
    const handleExportExcel = () => {
        ...
        reportData.forEach((item) => {
            excelData.push([
                ...
                item.available_stock_value.toFixed(2),
            ]);
        });
        excelData.push([
            '',
            'TOTAL',
            totals.total_medicines,
            totals.buy_qty,
            totals.buy_total.toFixed(2),
            totals.sale_qty,
            totals.sale_total.toFixed(2),
            totals.total_profit.toFixed(2),
            totals.available_stock_qty,
            totals.available_stock_value.toFixed(2),
        ]);
        ...
        <AdminLayout>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
                ...
                <div className="mb-3 flex gap-2">
                    <button onClick={() => window.print()} className="rounded-md bg-gray-600 px-3 py-1.5 text-sm text-white hover:bg-gray-700">
                        Print
                    </button>
                    ...
                    <table className="min-w-full border-collapse">
                        <thead>
                            <tr className="bg-gray-100">
                                ...
                                <th className="border border-gray-300 bg-gray-200 px-3 py-2 text-center text-xs font-bold text-gray-800">
                                    AVAILABLE STOCK (QTY)
                                </th>
                                <th className="border border-gray-300 bg-gray-200 px-3 py-2 text-center text-xs font-bold text-gray-800">
                                    AVAILABLE VALUE
                                    ...
                                    {reportData.map((item) => (
                                        <tr key={item.manufacturer} className="hover:bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 px-3 py-2 text-right text-sm text-gray-700">
                                                {item.buy_total > 0 ? item.buy_total.toFixed(2) : '-'}
                                            </td>
                                            <td className="border border-gray-300 px-3 py-2 text-center text-sm text-gray-700">
                                                {item.sale_qty > 0 ? item.sale_qty : '-'}
                                            </td>
                                            <td className="border border-gray-300 px-3 py-2 text-right text-sm text-gray-700">
                                                {item.sale_total > 0 ? item.sale_total.toFixed(2) : '-'}
                                            </td>
                                            <td className="border border-gray-300 px-3 py-2 text-right text-sm text-gray-700">
                                                {item.total_profit > 0 ? item.total_profit.toFixed(2) : '-'}
                                            </td>
                                            <td className="border border-gray-300 bg-gray-50 px-3 py-2 text-center text-sm font-bold text-gray-900">
                                                {item.available_stock_qty}
                                            </td>
                                            <td className="border border-gray-300 bg-gray-50 px-3 py-2 text-right text-sm font-bold text-gray-900">
                                                {item.available_stock_value.toFixed(2)}
                                            </td>
                                        </tr>
                                    ))}
                                    <tr className="bg-gray-100 font-bold">
                                        <td colSpan={2} className="border border-gray-300 px-3 py-2 text-center text-sm text-gray-900">
                                            TOTAL
                                        </td>
                                        ...
                                        <td className="border border-gray-300 bg-gray-200 px-3 py-2 text-center text-sm font-bold text-gray-900">
                                            {totals.available_stock_qty}
                                        </td>
                                        <td className="border border-gray-300 bg-gray-200 px-3 py-2 text-right text-sm font-bold text-gray-900">
                                            {totals.available_stock_value.toFixed(2)}
                                        </td>
                                    </tr>
                                </>
