...
import { Calendar, DollarSign, Edit as EditIcon, Eye, FileText, Filter, Printer, Receipt, Search, Stethoscope, Trash2, Users } from 'lucide-react';
...
export default function Index({ visits, doctors, filters }: Props) {
    ...
    const getStatusBadge = (status: string | null | undefined) => {
        if (!status) {
            return <span className="rounded bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-700">N/A</span>;
        }
        ...
        return (
            <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${statusColors[status] || 'bg-gray-100 text-gray-700'}`}>
                ...
                {showFilters && (
                    <div className="mb-4 rounded border border-gray-200 bg-gray-50 p-3">
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-3 lg:grid-cols-5">
                            <div>
                                <label className="mb-1 block text-xs font-medium text-gray-700">Payment Status</label>
                                <select
                                    value={paymentStatus}
                            ...
                            <div>
                                <label className="mb-1 block text-xs font-medium text-gray-700">Overall Status</label>
                                <select
                                    value={overallStatus}
                                ...
                                <tr key={visit.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-3 py-2">
                                        {visit.selected_doctor ? (
                                            <div className="flex flex-col gap-1">
                                                ...
                                                {visit.is_followup && (
                                                    <span className="inline-flex items-center rounded bg-emerald-100 px-1.5 py-0.5 text-xs font-medium text-emerald-800">
                                                        Follow-up
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    <td className="px-3 py-2">
                                        <div className="space-y-0.5">
                                            <div className="flex items-center gap-1">{getStatusBadge(visit.payment_status)}</div>
                                            <div className="flex items-center gap-1 text-xs text-gray-600">
                                                <DollarSign className="h-2.5 w-2.5" />
                                                <span>Total: {formatCurrency(visit.final_amount)}</span>
                                            </div>
                                            <p className="text-xs text-green-600">Paid: {formatCurrency(visit.total_paid)}</p>
                                            {visit.total_due && visit.total_due > 0 && (
                                                <p className="text-xs text-red-600">Due: {formatCurrency(visit.total_due)}</p>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-3 py-2 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            <button
                                                onClick={() => {
                                                    window.open(route('patients.download-blank-prescription', visit.patient.id), '_blank');
                                                }}
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-yellow-600 hover:bg-yellow-50 hover:text-yellow-800"
                                            >
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-green-600 hover:bg-green-50 hover:text-green-800"
                                            >
                                            ...
                                            <button
                                                onClick={() => {
                                                    if (
                                                        confirm(
                                                            `Delete Visit ${visit.visit_id}? This will reverse all transactions and cannot be undone.`,
                                                        )
                                                    ) {
                ...
                {visits.total > visits.per_page && (
                    <div className="mt-3 flex items-center justify-between">
                        <div className="text-xs text-gray-700">
                            ...
                            {Math.min(visits.current_page * visits.per_page, visits.total)} of {visits.total} results
                        </div>
