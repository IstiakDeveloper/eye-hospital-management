...
export default function BuySaleStockReport({ reportData: rawReportData, totals: rawTotals, opticsTotals, filters }: Props) {
    ...
    const handleFilter = () => {
        router.get(
            route('optics.reports.buy-sale-stock'),
            {
                from_date: fromDate,
                to_date: toDate,
    ...
    const handleExportExcel = () => {
        ...
        // Export
        XLSX.writeFile(wb, `Optics_Buy_Sale_Stock_Report_${fromDate}_to_${toDate}.xlsx`);
    };

    return (
        <AdminLayout>
            <Head title="Buy-Sale-Stock Report" />
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="glasses">Frames Only</option>
                                <option value="lens_types">Lens Only</option>
                                ...
                                <>
                                    {reportData.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="border border-gray-300 bg-yellow-50 px-1 py-2 text-right text-sm text-gray-900">
                                                {item.sale_fitting > 0 ? item.sale_fitting.toFixed(2) : '-'}
                                            </td>
                                            <td className="border border-gray-300 bg-yellow-50 px-2 py-2 text-right text-sm text-gray-900">
                                    ...
                                    <tr className="bg-orange-50 font-semibold">
                                        ...
                                        {/* Sale - empty except fitting */}
                                        <td className="border border-gray-300 bg-yellow-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                        <td className="border border-gray-300 bg-yellow-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                        <td className="border border-gray-300 bg-yellow-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                        <td className="border border-gray-300 bg-yellow-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                        {/* Fitting column - show only fitting charge */}
                                        ...
                                        <td className="border border-gray-300 bg-pink-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                        <td className="border border-gray-300 bg-orange-100 px-2 py-3 text-right text-sm font-bold text-green-600">
                                            {totals.only_fitting_charge.toFixed(2)}
                                        </td>

                                        {/* Available - empty */}
                                        <td className="border border-gray-300 bg-purple-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                        <td className="border border-gray-300 bg-purple-50 px-2 py-3 text-center text-sm text-gray-500">-</td>
                                    </tr>
