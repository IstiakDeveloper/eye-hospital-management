        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                    <StatCard
                        ...
                        color="bg-green-500"
                    />
                    <StatCard title="Today's Sales" value={`-${stats.today_sales}`} subtitle="Items sold" icon={ShoppingCart} color="bg-blue-500" />
                    <StatCard
                        title="Adjustments"
                ...
                <div className="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    {hasActiveFilters && (
                        <div className="mt-3 flex items-center gap-2 text-sm">
                            ...
                            {filters.type && <span className="rounded bg-blue-100 px-2 py-1 text-blue-700">{filters.type}</span>}
                            {filters.item_type && (
                                <span className="rounded bg-green-100 px-2 py-1 text-green-700">{getItemTypeLabel(filters.item_type).label}</span>
                            )}
                            {filters.date_from && <span className="rounded bg-purple-100 px-2 py-1 text-purple-700">From: {filters.date_from}</span>}
                            {filters.date_to && <span className="rounded bg-purple-100 px-2 py-1 text-purple-700">To: {filters.date_to}</span>}
                        </div>
                    )}
                </div>
                ...
                <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
                                ...
                                {movements.data.map((movement: StockMovement) => {
                                    ...
                                    return (
                                        <tr key={movement.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4">
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        <p className="font-medium text-gray-900">{movement.item_brand}</p>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className={`rounded p-1.5 ${movementConfig.color}`}>
                                                        <MovementIcon className="h-4 w-4" />
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-medium text-gray-900">{movementConfig.label}</p>
                                                        <p className="text-xs text-gray-500">{movementConfig.description}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium text-gray-900">{formatCurrency(movement.unit_price)}</td>
                                            <td className="px-6 py-4">
                                                <p className="text-base font-bold text-gray-900">{formatCurrency(movement.total_amount)}</p>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div>
                                                    <p className="text-sm text-gray-900">{formatDate(movement.created_at)}</p>
                                                    <p className="mt-0.5 text-xs text-gray-500">By: {movement.user.name}</p>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {canDelete(movement) && (
                                                        <Button variant="danger" size="sm" onClick={() => handleDelete(movement)}>
                    ...
                    {movements.data.length === 0 && (
                        <div className="py-12 text-center">
                            ...
                            <h3 className="mb-2 text-lg font-medium text-gray-900">No movements found</h3>
                            <p className="text-gray-500">
                                {hasActiveFilters ? 'Try adjusting your filters' : 'Start by adding stock to see movements here'}
                            </p>
                        </div>
                    )}
                </div>
            ...
            <Modal isOpen={deleteModal.isOpen} onClose={() => setDeleteModal({ isOpen: false })}>
                <div className="text-center">
                    ...
                    <h3 className="mb-2 text-lg font-medium text-gray-900">Delete Stock Movement?</h3>
                    ...
                    {deleteModal.movement && (
                        <div className="mb-6 rounded-lg bg-gray-50 p-4 text-left text-sm">
                            <div className="space-y-2">
                                <p>
                                    <strong>Product:</strong> {deleteModal.movement.item_brand} {deleteModal.movement.item_model}
                                </p>
                                <p>
                                    <strong>SKU:</strong> {deleteModal.movement.item_sku}
                                </p>
                                <p>
                                    <strong>Quantity:</strong> {deleteModal.movement.quantity}
                                </p>
                                <p>
                                    <strong>Amount:</strong> {formatCurrency(deleteModal.movement.total_amount)}
                                </p>
                            </div>
                        </div>
                    )}
                    <div className="flex gap-3">
                        <Button variant="secondary" onClick={() => setDeleteModal({ isOpen: false })} className="flex-1">
                            Cancel
                        </Button>
                        <Button variant="danger" onClick={confirmDelete} disabled={processing} className="flex-1">
