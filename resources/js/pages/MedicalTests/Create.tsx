...
export default function MedicalTestCreate({ tests, paymentMethods, categories }: {
    ...
    const handleDiscountChange = (testId: number, discount: number) => {
        setSelectedTests(selectedTests.map(t => {
            if (t.medical_test_id === testId) {
                const discountAmount = Math.min(Math.max(0, discount), t.test.price);
                return {
                    ...t,
                    discount_amount: discountAmount,
                    final_price: t.test.price - discountAmount
                };
            }
            return t;
        }));
    ...
    const totalOriginal = selectedTests.reduce((sum, t) => sum + t.test.price, 0);
    const totalDiscount = selectedTests.reduce((sum, t) => sum + t.discount_amount, 0);
    const totalFinal = selectedTests.reduce((sum, t) => sum + t.final_price, 0);
    const dueAmount = Math.max(0, totalFinal - paidAmount);

    // Auto-set paid amount to total when tests change
    useEffect(() => {
        if (totalFinal > 0 && paidAmount === 0) {
            setPaidAmount(totalFinal);
        }
    }, [totalFinal]);
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center justify-between mt-3">
                                                    ...
                                                    <span className="text-xl font-bold text-blue-600">৳{Number(test.price).toFixed(2)}</span>
                                                </div>
                                                {test.description && (
                                        ...
                                        <table className="w-full">
                                            ...
                                            <tbody className="divide-y divide-gray-200">
                                                {selectedTests.map(item => (
                                                    <tr key={item.medical_test_id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-4 py-3 text-right text-gray-900">৳{Number(item.test.price).toFixed(2)}</td>
                                                        <td className="px-4 py-3">
                                                            <div className="flex items-center justify-end gap-2">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => handleDiscountChange(item.medical_test_id, item.discount_amount - 10)}
                                            ...
                                            <tfoot className="bg-gray-50 font-semibold">
                                                <tr>
                                                    ...
                                                    <td className="px-4 py-3 text-right text-gray-900">৳{Number(totalOriginal).toFixed(2)}</td>
                                                    <td className="px-4 py-3 text-right text-red-600">-৳{Number(totalDiscount).toFixed(2)}</td>
                                                    <td className="px-4 py-3 text-right text-xl text-blue-600">৳{Number(totalFinal).toFixed(2)}</td>
                                                    <td></td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                ...
                                <div className="bg-white rounded-lg shadow p-6 mb-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setPaidAmount(totalFinal)}
                                                className="mt-2 text-sm text-blue-600 hover:text-blue-800 font-medium"
                                            >
                                                Pay Full Amount (৳{Number(totalFinal).toFixed(2)})
                                            </button>
                                        </div>
                                    ...
                                    <div className="mt-6 p-6 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg border-2 border-blue-200">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div className="text-center">
                                                ...
                                                <div className="text-3xl font-bold text-gray-900">৳{Number(totalFinal).toFixed(2)}</div>
                                            </div>

                                            <div className="text-center">
                                                <div className="text-sm text-gray-600 mb-1">Paid Amount</div>
                                                <div className="text-3xl font-bold text-green-600">৳{Number(paidAmount).toFixed(2)}</div>
                                            </div>

                                            <div className="text-center">
                                                <div className="text-sm text-gray-600 mb-1">Due Amount</div>
                                                <div className="text-3xl font-bold text-red-600">৳{Number(dueAmount).toFixed(2)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
