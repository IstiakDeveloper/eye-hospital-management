...
import { ChevronLeft, ChevronRight, FileText, Filter, TrendingDown, TrendingUp } from 'lucide-react';
...
const Transactions: React.FC<Props> = ({ transactions, balance }) => {
    ...
    const applyFilters = () => {
        router.get('/operation-account/transactions', filters, { preserveState: true });
    };

    const resetFilters = () => {
        setFilters({ type: '', start_date: '', end_date: '' });
        router.get('/operation-account/transactions', {}, { preserveState: true });
    };

    return (
        <OperationAccountLayout title="Transactions">
            {/* Balance Card */}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="border-b bg-gray-50">
                            <tr>
                                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Transaction No</th>
                                ...
                                transactions.data.map((transaction) => (
                                    <tr key={transaction.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4">
                                            <span className="font-mono text-sm">{transaction.transaction_no}</span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="line-clamp-2 text-sm text-gray-600">{transaction.description}</span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-600">{transaction.created_by.name}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
                    ...
                    <div className="border-t bg-gray-50 px-6 py-4">
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-gray-700">
                                Showing <span className="font-medium">{transactions.data.length}</span> of{' '}
