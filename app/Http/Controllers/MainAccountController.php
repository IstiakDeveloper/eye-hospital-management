...
class MainAccountController extends Controller
{
    ...
    public function receiptAndPaymentReport(Request $request): Response
    {
        $validated = $request->validate([
            ...
            'date_from' => 'nullable|date',
            'date_to' => 'nullable|date|after_or_equal:date_from',
            'report_type' => 'nullable|in:monthly,custom', // monthly or custom date range
        ]);
        ...
        if ($reportType === 'monthly') {
            ...
            $reportTitle = Carbon::createFromDate($year, $month)->format('F Y');
        } else {
            // Custom date range
            $startDate = $validated['date_from'] ?? now()->startOfMonth();
            $endDate = $validated['date_to'] ?? now()->endOfMonth();

            $reportTitle = Carbon::parse($startDate)->format('d/m/Y') . ' to ' . Carbon::parse($endDate)->format('d/m/Y');
        }
        ...
        return Inertia::render('MainAccount/ReceiptAndPaymentReport', [
            ...
            'hospital_name' => 'Naogaon Islamia Eye Hospital and Phaco Center',
            'hospital_location' => 'Naogaon',
            'month' => $reportType === 'monthly' ? ($validated['month'] ?? now()->month) : null,
            'year' => $reportType === 'monthly' ? ($validated['year'] ?? now()->year) : null,
        ]);
    }
    ...
    public function incomeExpenditureReport(Request $request): Response
    {
        $validated = $request->validate([
            ...
            'date_from' => 'nullable|date',
            'date_to' => 'nullable|date|after_or_equal:date_from',
            'report_type' => 'nullable|in:monthly,custom',
        ]);
        ...
        if ($reportType === 'monthly') {
            ...
            $prevStartDate = Carbon::createFromDate($previousYear, $previousMonth, 1)->startOfMonth();
            $prevEndDate = Carbon::createFromDate($previousYear, $previousMonth, 1)->endOfMonth();

            $prevIncome = MainAccountVoucher::whereBetween('date', [$prevStartDate, $prevEndDate])
                ->where('voucher_type', 'Credit')
                ->whereIn('source_transaction_type', ['income', 'other_income', 'bank_interest'])
                ->sum('amount');

            $prevExpenditure = MainAccountVoucher::whereBetween('date', [$prevStartDate, $prevEndDate])
                ->where('voucher_type', 'Debit')
                ->where('source_transaction_type', 'expense')
                ->sum('amount');

            $monthlyComparison = [
                'previous_month' => Carbon::createFromDate($previousYear, $previousMonth)->format('F Y'),
                'previous_income' => (float) $prevIncome,
                'previous_expenditure' => (float) $prevExpenditure,
                'previous_surplus_deficit' => (float) ($prevIncome - $prevExpenditure),
                'income_growth' => $prevIncome > 0 ? (($totalIncome - $prevIncome) / $prevIncome) * 100 : 0,
                'expenditure_growth' => $prevExpenditure > 0 ? (($totalExpenditure - $prevExpenditure) / $prevExpenditure) * 100 : 0,
                'formatted_previous_income' => number_format($prevIncome, 2),
                'formatted_previous_expenditure' => number_format($prevExpenditure, 2),
                'formatted_previous_surplus_deficit' => number_format($prevIncome - $prevExpenditure, 2),
            ];
        }
