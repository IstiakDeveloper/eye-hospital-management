...
class MedicalTestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($validated['paid_amount'] > 0) {
                ...
                // 2. Update Hospital Account + Create Hospital Transaction
                $hospitalAccount = HospitalAccount::firstOrCreate([]);
                $hospitalAccount->increment('balance', $validated['paid_amount']);

                $hospitalTransaction = HospitalTransaction::create([
                    'transaction_no' => 'HT-' . date('Ymd') . '-' . str_pad(rand(1, 9999), 4, '0', STR_PAD_LEFT),
                    'type' => 'income',
                    'amount' => $validated['paid_amount'],
                    'category' => 'Medical Test',
                    'reference_type' => 'medical_test',
                    'reference_id' => $testGroup->id,
    ...
    public function addPayment(Request $request, PatientTestGroup $testGroup)
    {
        ...
        try {
            ...
            // 3. Update Hospital Account + Create Hospital Transaction
            $hospitalAccount = HospitalAccount::firstOrCreate([]);
            $hospitalAccount->increment('balance', $validated['amount']);
